...
interface UseKeyPressOptions {
  ...
  preventInInputs?: boolean; // New option to prevent shortcuts when typing in inputs
}
...
function isTypingInInput(): boolean {
  ...
  // Check if it's inside an element with contentEditable
...
export function useKeyPress(
  ...
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      // Prevent shortcuts when typing in input fields (unless disabled)
      if (preventInInputs && isTypingInInput()) {
        return;
      }
  ...
  }, [key, target, event, eventOptions, preventInInputs]);
}